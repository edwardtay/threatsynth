...
// List briefings with threat + asset data
briefings.get('/', async (c) => {
  const rows = await c.env.DB.prepare(`
    SELECT b.*,
      t.source_id as threat_source_id, t.severity as threat_severity, t.title as threat_title, t.source as threat_source,
  ...
  const mapped = rows.results.map((r: any) => ({
    ...
    threat: {
      id: r.threat_id,
      source_id: r.threat_source_id,
      severity: r.threat_severity,
      title: r.threat_title,
      source: r.threat_source,
    },
    asset: {
      id: r.asset_id,
      name: r.asset_name,
      version: r.asset_version,
      vendor: r.asset_vendor,
    },
  }));
...
// Generate briefings using Workers AI
briefings.post('/generate', async (c) => {
  ...
  if (assets.results.length === 0) return c.json({ detail: 'No assets registered. Add assets first.' }, 400);
  ...
  for (const asset of assets.results) {
    ...
    // Find threats matching this asset's vendor/product
    const matching = threats.results.filter((t: any) => {
      if (!a.vendor && !a.product) return false;
      const vendorMatch = a.vendor && t.affected_vendor &&
        a.vendor.toLowerCase().includes(t.affected_vendor.toLowerCase());
      const productMatch = a.product && t.affected_product &&
        a.product.toLowerCase().includes(t.affected_product.toLowerCase());
      return vendorMatch || productMatch;
    });

    // If no direct matches, take top 2 critical threats for awareness
    ...
    for (const threat of toProcess) {
      ...
      // Generate briefing with Workers AI
...
IMPACT: [1-2 sentences on business impact]`;
        ...
        const summaryMatch = text.match(/SUMMARY:\s*([\s\S]*?)(?=REMEDIATION:|$)/i);
        const remediationMatch = text.match(/REMEDIATION:\s*([\s\S]*?)(?=IMPACT:|$)/i);
        const impactMatch = text.match(/IMPACT:\s*([\s\S]*?)$/i);

        summary = summaryMatch?.[1]?.trim() || text.slice(0, 200);
      ...
      await c.env.DB.prepare(`INSERT INTO briefings (threat_id, asset_id, summary, remediation, business_impact, priority_score)
        VALUES (?, ?, ?, ?, ?, ?)`)
        .bind(t.id, a.id, summary, remediation, businessImpact, Math.round(priority * 10) / 10)
        .run();
      generated++;
    }
  }
